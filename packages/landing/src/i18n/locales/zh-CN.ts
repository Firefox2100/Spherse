export const zhCN = {
  ...
  "hero.linuxTip": "Linux 版为 AppImage 格式，下载后需先赋予可执行权限再运行（终端执行 chmod +x Spherse-*.AppImage，或在文件管理器属性中勾选「允许作为程序执行」）。Ubuntu 23.10 及以上版本若启动时报 SUID sandbox 错误，请先执行 sudo sysctl -k kernel.apparmor_restrict_unprivileged_userns=0 后重试；deb 安装包可前往 GitHub Releases 下载。",
